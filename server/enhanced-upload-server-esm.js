// ES Modules version of enhanced upload server
// Use: node server/enhanced-upload-server-esm.js
...
// Import storage from JavaScript version
...
// Album upload endpoint that matches patched-album-upload.js
  ...
  albumUpload(req, res, async (err) => {
    ...
    try {
      ...
      const album = await storage.createAlbum({
        ...
        coverUrl: '/covers/default-cover.jpg', // Will be updated
      ...
      // Return success response with album and track count
      res.status(200).json({
        album: {
          id: album.id,
          title: album.title,
...
app.listen(port, '0.0.0.0', () => {
  console.log(`Enhanced upload server (ESM) running on port ${port}`);
